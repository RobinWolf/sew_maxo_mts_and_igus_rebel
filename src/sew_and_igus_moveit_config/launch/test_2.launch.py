...
# load planning configuration related yaml files
    ...
    controllers_yaml = load_yaml(Path(controllers.perform(context)))
    ...
    robot_description_kinematics_file = PathJoinSubstitution([FindPackageShare(moveit_package),"config","kinematics.yaml"])
    ...
    ompl_planning_yaml = {"ompl": load_yaml(Path(ompl_file.perform(context)))}
    ...
    rviz_file = PathJoinSubstitution([FindPackageShare(moveit_package), "rviz", "moveit_rviz.rviz"]) # define path to rviz-config file
    rviz_node = Node(
        package="rviz2",
        executable="rviz2",
        name="rviz2",
